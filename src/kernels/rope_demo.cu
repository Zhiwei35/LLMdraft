...
__global__ void rotary_embedding_neox_kernel(
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) { // threads负责hidden units
    const int head_idx = i / embed_dim;
    const int token_head = token_idx * stride + head_idx * head_size;

    const int rot_offset = i % embed_dim; // rotatesize=64
    const int x_index = rot_offset; // 0-63
    const int y_index = embed_dim + rot_offset; // 64-127 
...
void rotary_embedding_neox(
  ...
  torch::Tensor& query,             // [num_tokens, num_heads * head_size] / [bs, num_heads * head_size]
  torch::Tensor& key,               // [num_tokens, num_heads * head_size] / [bs, num_heads * head_size]
