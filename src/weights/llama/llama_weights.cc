...
void LlamaWeight<T>::loadWeights(std::string weight_path) {
    ...
    loadWeightFromBin<T, half>::internalFunc(out_rmsnorm_weight.gamma, {(size_t)hidden_units}, weight_path + "model.norm.weight.bin");
    std::cout << "loaded norm weight " << weight_path <<"\n";
    loadWeightFromBin<T, half>::internalFunc(post_decoder_embedding_weight.data, {(size_t)hidden_units, (size_t)vocab_size}, weight_path + "lm_head.weight.bin");
    std::cout << "loaded lmhead weight " << weight_path <<"\n";
    loadWeightFromBin<T, half>::internalFunc(pre_decoder_embedding_weight.data, {(size_t)vocab_size, (size_t)hidden_units}, weight_path + "model.embed_tokens.weight.bin");
