...
{
    // load out linear weight
    internalFunc(ptr, {4096, 4096}, weight_path + "model.layers.0.self_attn.o_proj.weight.bin");
    // load attn output
    internalFunc(ptr1, {13, 4096}, weight_path + "attn_output.bin");
...
int main(int argc, char *argv[]) {
    ...
    float* d_in = (float*) malloc(sizeof(float) * hidden_units * seqlen);
    float* d_in_trans = (float*) malloc(sizeof(float) * hidden_units * seqlen);
    ...
    loadWeights_trans(d_w_trans, d_in_trans, "/home/");
    std::cout << "====weight====" << "\n";
    //CheckResult(d_w, d_w_trans, hidden_units_2);
    std::cout << "====input====" << "\n";
